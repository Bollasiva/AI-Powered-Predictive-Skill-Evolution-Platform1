...
import User, { ISkill, IUser } from '../models/user';
import Trend from '../models/Trend';
import resourceMap from '../data/resourceMap';
import careerPaths from '../data/careerPaths';
import SKILL_PREREQUISITES from '../data/skillPrerequisites';
import { getSemanticSkills } from '../services/aiService';

const PROFICIENCY_WEIGHTS: Record<string, number> = { 'Beginner': 1, 'Intermediate': 2, 'Advanced': 3, 'Expert': 4 };

const calculateWeightedSimilarity = (currentSkills: ISkill[], otherSkills: ISkill[]): number => {
...
export const getMarketRecommendations = async (req: Request, res: Response) => {
  try {
    const { skills: userSkills, careerInterest } = req.body as { skills: ISkill[], careerInterest: string };
    ...
    const trendsMap = new Map(trendsData.trends.map((s: any) => [s.skill.toLowerCase(), s]));
...
export const getCollaborativeRecommendations = async (req: Request, res: Response) => {
  try {
    ...
    const otherUsers = await User.find({ _id: { $ne: req.user.id } }).select('skills').limit(100) as unknown as IUser[];
